		...
		foreach ( $post_types as $post_slug => $type ) {
			...
			if (  ! empty($posts_array) ) {
				...
				if ( $this->oc_premium() === true ) {
					...
					$html .= sprintf('<option value="%s"%s disabled="disabled">%s</option>', "all-".$post_slug, selected($current_value, "all-".$post_slug, false), __('All', ONECOM_UC_TEXT_DOMAIN).' '.$type->labels->name);
				}
