...
class OCUC_Toolbar {
	...
	public function add_toolbar_link() {
		...
		if ( isset($uc_option['uc_status']) && $uc_option['uc_status'] === 'on' ) {
			...
			$uc_status       = sprintf(__('Maintenance Mode is %sON%s', ONECOM_UC_TEXT_DOMAIN), '<span class="last_on">', '</span>');
			$uc_meta_title   = sprintf(__('Maintenance Mode is %sON%s', ONECOM_UC_TEXT_DOMAIN), '', '');
		} else {
			$uc_status_class = 'ocuc_toggle_settings ocuc_setting_is_off';
			$uc_status       = __('Maintenance Mode is OFF', ONECOM_UC_TEXT_DOMAIN);
			$uc_meta_title   = __('Maintenance Mode is OFF', ONECOM_UC_TEXT_DOMAIN);
		}
