...
<div class="notice notice-success advanced-settings oc_hidden">
	<p><?php _e('Advanced settings updated!', OC_SP_TEXTDOMAIN) ?></p></div>
<form id="sp-advanced-settings" class="sp-blocked-lists" name="sp-advanced-settings" method="post">
	...
	<p class="description"><textarea name="oc_whitelist_usernames" class="oc_whitelist_usernames" cols="62" rows="8"><?php
        ...
        echo $username . "\r\n";
			...
			?></textarea></br><span><em><?php _e(OnecomSp::ONE_ENTRY_PER_LINE, OC_SP_TEXTDOMAIN); ?></em></span>
	</p></br>
	...
	<p class="description">
		<span class="prt-desc"> <?php _e('Browsers always include a user agent string when they access a site. If you want to whitelist any user agents then you can add them here.', OC_SP_TEXTDOMAIN) ?></span>
	</p>
	...
	<label for="spwhitelistIP">
			...
			<input class="oc_sp_check" type="checkbox" id="spwhitelistIP" name="oc_spwhitelistip"
				   value="true" <?php if ( $oc_whitelistip == 'true' ) {
                        echo OnecomSp::CHECKED;
			} ?>>
				<span class="oc_sp_slider"></span>
			</span>
		<span><?php _e('Whitelist IP address', OC_SP_TEXTDOMAIN) ?></span></label>
	<p class="description">
		<span class="prt-desc"> <?php _e('Here you can add specific IP addresses that you want to whitelist.', OC_SP_TEXTDOMAIN) ?></span>
	</p>
	...
	<p class="description">
		<span class="prt-desc"><?php _e('This checks for URL shorteners listed below, in emails, author fields and comment body. Any form submission request having URL shortener will be blocked. You can add/delete shorteners from the list.', OC_SP_TEXTDOMAIN) ?></span>
	</p>
	...
	<p class="description">
		<span class="prt-desc">  <?php _e('Bots(automated programs) randomly search websites for exploitable files. If a URL is not found on the website(returns 404) and it is a match to a exploit URL from the list then the request will be blocked.', OC_SP_TEXTDOMAIN) ?></span>
	</p>
	...
	<p class="description"><textarea name="oc_exploit_urls" class="oc-exploit-urls" cols="62" rows="8"><?php
        ...
        print $exploits . "\n";
			...
			?></textarea></br><span><em><?php _e(OnecomSp::ONE_ENTRY_PER_LINE, OC_SP_TEXTDOMAIN); ?></em></span></p></br>
	<p>
