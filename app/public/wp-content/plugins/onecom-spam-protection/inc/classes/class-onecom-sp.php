...
class OnecomSp {
	...
	public function ocsp_wp_load_textdomain() {
		...
		if ( $current_locale === 'fi' ) {
            load_textdomain(OC_SP_TEXTDOMAIN, ONECOM_PLUGIN_PATH . '/languages/onecom-sp-fi_FI.mo');
		} else if ( $current_locale === 'nb_NO' ) {
			load_textdomain(OC_SP_TEXTDOMAIN, ONECOM_PLUGIN_PATH . '/languages/onecom-sp-no_NO.mo');
			...
			load_textdomain(OC_SP_TEXTDOMAIN, ONECOM_PLUGIN_PATH. '/languages/onecom-sp-en_GB.mo');
		}
	}
	...
	public function admin_styles() {
		...
		$bodylist     = [
			__("Protects the website from spam comments, spam registration, spambots, and other spammers.", OC_SP_TEXTDOMAIN),
			__("Customizable protection settings, spam logs, and other options.", OC_SP_TEXTDOMAIN),
		];
		$popupContent = [
			"title"       => sprintf(__("Get Spam Protection plugin with %s Managed WordPress", OC_SP_TEXTDOMAIN), "\n"),
			"top-desc"    => __("Spend less time worrying about your site’s safety and more time growing your business with one.com Managed WordPress.", OC_SP_TEXTDOMAIN),
			"footer-desc" => __("Managed WordPress includes other great features such as Vulnerability Monitor and Upgrade Manager", OC_SP_TEXTDOMAIN),
			"bodylist"    => $bodylist
		];
		...
		if ( in_array($screen_id, $this->onecom_sp_allowed_screens()) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'generic_validation_msg'      => __("The input doesn't match the allowed format for this field. Please check and try again.", OC_SP_TEXTDOMAIN),
					'failed_login_validation_msg' => __("The number of failed login attempts must be between 1-10. The time in seconds should have a value between 10 and 900. Please check the numbers you entered and try again.", OC_SP_TEXTDOMAIN)
				)
			);
		}
