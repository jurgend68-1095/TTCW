...
if ( ! defined('ONECOM_WP_VERSION') ) {
	define('ONECOM_WP_VERSION', '4.4.3');
}
...
if ( ! function_exists('onecom_plugin_upgradation') ) {
	function onecom_plugin_upgradation($upgrader_object, $options) {
		...
		//remove login_protection from previous scan
		oc_unset_login_protection();
...
/**
 * @return void
 * function to unset login protection from previous HM scan
 * @todo remove it after next major update i.e 5.0
 */
if(!function_exists('oc_unset_login_protection')){

	function oc_unset_login_protection(){
		$prev_scan = get_site_transient('ocsh_site_previous_scan');
		if ($prev_scan && is_array($prev_scan) && isset($prev_scan['login_protection'])) {
			unset($prev_scan['login_protection']);
