...
class OnecomHealthMonitorAjax extends OnecomHealthMonitor {
	...
	public function add_check_callbacks(): void {
		...
		add_action('wp_ajax_ocsh_check_debug_enabled', [$this, 'debug_enabled']);
		add_action('wp_ajax_ocsh_check_debug_log_size', [$this, 'debug_log_size']);
		...
		add_action('wp_ajax_ocsh_check_inactive_plugins', [$this, 'inactive_plugins']);
		add_action('wp_ajax_ocsh_check_inactive_themes', [$this, 'inactive_themes']);
	}
	...
	public function send_json(array $result, string $check = '') {
		if ( ! (defined('REST_REQUEST') || defined('DOING_CRON')) ) {
			...
			}else if ( 0 === $result[ $this->status_key ] && $this->is_ignored($check) ) {
